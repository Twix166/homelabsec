...
EDGE_RUNTIME_ENV_PRESENT=false
if [[ -f "$ROOT_DIR/.edge-runtime.env" ]]; then
  EDGE_RUNTIME_ENV_PRESENT=true
  set -a
...
API_HOST_PORT="${API_HOST_PORT:-${BRAIN_HOST_PORT:-8088}}"
FRONTEND_HOST_PORT="${FRONTEND_HOST_PORT:-${HOMELABSEC_UAT_FRONTEND_PORT:-8080}}"
EDGE_AUTH_MODE="${EDGE_AUTH_MODE:-}"

if [[ "$EDGE_RUNTIME_ENV_PRESENT" == "true" && -n "$EDGE_AUTH_MODE" ]]; then
